...
class Widevine:
    ...
    def get_content_keys(self, cdm: WidevineCdm, certificate: Callable, license: Callable) -> None:
        """
        ...
        for kid in self.kids:
            ...
            try:
                ...
                license_challenge = cdm.get_license_challenge(session_id, self.pssh)

                print("license_challenge:", license_challenge)
    ...
    def decrypt(self, path: Path) -> None:
        ...
        Raises:
            EnvironmentError if the mp4decrypt executable could not be found.
            ValueError if the track has not yet been downloaded.
            SubprocessError if mp4decrypt returned a non-zero exit code.
        """
        if not self.content_keys:
        ...
        try:
            keys = [f"--key \"{kid.hex}:{key.lower()}\"" for i, (kid, key) in enumerate(self.content_keys.items())]
            
            p = subprocess.Popen(
                f'{binaries.MP4Decrypt} --show-progress {" ".join(keys)} "{path}" "{output_path}"',
